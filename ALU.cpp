...
void ALU::ADDI(Registers& mstate, uint8_t rd, uint8_t rs1, uint16_t imm){
  mstate.set(rd, mstate.get(rs1) + imm);
}

void ALU::XORI(Registers& mstate, uint8_t rd, uint8_t rs1, uint16_t imm){
  mstate.set(rd, mstate.get(rs1) ^ imm);
}

void ALU::ORI(Registers& mstate, uint8_t rd, uint8_t rs1, uint16_t imm){
  mstate.set(rd, mstate.get(rs1) | imm);
}

void ALU::ANDI(Registers& mstate, uint8_t rd, uint8_t rs1, uint16_t imm){
  mstate.set(rd, mstate.get(rs1) & imm);
}

void ALU::SLLI(Registers& mstate, uint8_t rd, uint8_t rs1, uint16_t imm){
  imm &= 0x000F;
  mstate.set(rd, mstate.get(rs1) << imm);
}
...
void ALU::SLTI(Registers& mstate, uint8_t rd, uint8_t rs1, uint16_t imm){
  mstate.set(rd, ((int32_t)mstate.get(rs1) < (int16_t)imm) ? 1 : 0);
}

void ALU::SLTIU(Registers& mstate, uint8_t rd, uint8_t rs1, uint16_t imm){
  mstate.set(rd, ((uint32_t)mstate.get(rs1) < (uint16_t)imm) ? 1 : 0);
}

void ALU::BEQ(Registers& mstate, uint8_t rs1, uint8_t rs2, int16_t imm){
	if (mstate.get(rs1) == mstate.get(rs2)) {
		mstate.set_pc(mstate.get_pc() + imm);
	}
}

void ALU::BNE(Registers& mstate, uint8_t rs1, uint8_t rs2, int16_t imm){
	if (mstate.get(rs1) != mstate.get(rs2)) {
		mstate.set_pc(mstate.get_pc() + imm);
	}
}

void ALU::BLT(Registers& mstate, uint8_t rs1, uint8_t rs2, int16_t imm){
	if ((int32_t)mstate.get(rs1) < (int32_t)mstate.get(rs2)) {
		mstate.set_pc(mstate.get_pc() + imm);
	}
}

void ALU::BGE(Registers& mstate, uint8_t rs1, uint8_t rs2, int16_t imm){
	if ((int32_t)mstate.get(rs1) >= (int32_t)mstate.get(rs2)) {
		mstate.set_pc(mstate.get_pc() + imm);
	}
}

void ALU::BLTU(Registers& mstate, uint8_t rs1, uint8_t rs2, int16_t imm){
	if (mstate.get(rs1) < mstate.get(rs2)) {
		mstate.set_pc(mstate.get_pc() + imm);
	}
}

void ALU::BGEU(Registers& mstate, uint8_t rs1, uint8_t rs2, int16_t imm){
	if (mstate.get(rs1) >= mstate.get(rs2)) {
		mstate.set_pc(mstate.get_pc() + imm);
	}
}
